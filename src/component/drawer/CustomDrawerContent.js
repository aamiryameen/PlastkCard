...
export default DrawerContent = (props) => {
  ...
  const getPictureOrAvatar = () => {
    ...
    if (response && response.user && response.user.hasOwnProperty('picture_url')) {
      return (
        <Avatar.Image size={60} source={{ uri: response.user.picture_url }} />
      )
    } else {

      if (response && response.user.gender === "M")
        return (<Avatar.Image size={60} source={require('../../assets/images/male.png')} backgroundColor="black" />)
      else
        return (<Avatar.Image size={60} source={require('../../assets/images/female.png')} />)
          ...
          <View style={styles.userInfoSection}>
            <View style={{ flexDirection: 'column', marginTop: 15, alignItems: "center", paddingTop: 20 }}>
              ...
              <View style={{ marginLeft: 15, flexDirection: 'column' }}>
                <Title style={[styles.title, { color: myTheme.colors.LABEL_COLOR }]}>{response.user.first_name + " " + response.user.last_name}</Title>
                <Caption style={[styles.caption, { color: myTheme.colors.LABEL_COLOR }]}>{response.user.email}</Caption>
              </View>
            </View>
            <View style={styles.btn_item_container}>
              <View style={styles.btn_container} >
                <GradientButton title="Home" titleStyle={{ color: myTheme.colors.LABEL_COLOR, fontFamily: mulish_bold }} style={styles.gradientBtnStyles} onPress={() => props.navigation.navigate("DashBoard")} />

              </View>
              <View style={styles.btn_container} >
                <GradientButton title="Profile" titleStyle={{ color: myTheme.colors.LABEL_COLOR, fontFamily: mulish_bold }} style={styles.gradientBtnStyles} onPress={() => props.navigation.navigate("Proflie")} />
              ...
              <View style={styles.btn_container}>
                <GradientButton title="Plastk Rewards Store" titleStyle={{ color: myTheme.colors.LABEL_COLOR, fontFamily: mulish_bold }} style={styles.gradientBtnStyles}
                  onPress={() => dispatch(fetchGRSUrlAction())} />

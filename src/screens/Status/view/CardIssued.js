...
export default FundRequested = (props) => {
  ...
  const dialHelpline = () => {
    openLink('tel:18442000524');
  };

  useEffect(() => {
    logFireBaseEvent('card_issued_screen');
  }, []);
  ...
  return (
    <View style={styles.bodyView}>
      ...
      <View
        ...
        <Image
          source={require('../../../assets/images/card-issued-filled.png')}
          resizeMode="cover"
          style={styles.image}
        />
      </View>
      ...
      <View style={[styles.fundRequestedTextContainer]}>
        ...
        <Text
          ...
          Your card is on its way and you should receive it in 10-15 business
          days.{' '}
        </Text>
        <Text
          style={[
            styles.moneyRequestText,
            {color: myTheme.colors.DARK_TEXT_COLOR, marginTop: normalizeY(10)},
          ]}>{`Once you have received your card you can activate it on this App or by calling `}</Text>
        <View style={{flexDirection: 'row'}}>
          <TouchableOpacity onPress={dialHelpline}>
            <Text
              style={[
                styles.moneyRequestText,
                {color: ligh_green, marginTop: normalizeY(2)},
              ]}>{`1-844-200-0524`}</Text>
          </TouchableOpacity>
        </View>

...
import { DASHBOARD_MY_CARD_DAILY, ligh_green, mulish_regular } from '../../../../utils/Constants';
...
export default MyCardDailyChart = (props) => {
    ...
    const isLoading = useSelector(state => state.myCardDailyReducer.isLoading)
    const response = useSelector(state => state.myCardDailyReducer.response)
    const isError = useSelector(state => state.myCardDailyReducer.isError)
    ...
    return (
        ...
        <LinearGradient colors={[myTheme.colors.CARD_GRADIENT_FIRST_COLOR, myTheme.colors.CARD_GRADIENT_SECOND_COLOR]} style={styles.linearGradientContainerStyle}>
            ...
            <View style={{ flexDirection: 'column', marginBottom: normalizeY(30), marginTop: normalizeY(5), marginHorizontal: normalizeX(15) }}>
                <Text style={[styles.titleTextStyle, { color: myTheme.colors.LABEL_COLOR, textAlign: 'center' }]}>Amount spent in the last 7 days</Text>
            ...
            <View style={{ position: 'absolute', justifyContent: 'center', alignItems: 'center', top: 25, right: 15 }}>
                <MaterialCommunityIcons
                    ...
                    onPress={() => props.navigation.navigate('TransactionList', { duration: '7' })}

                />
            </View>

            {response !== '' ? response.label && response.label.length > 0 ? <LineChartView response={response} view={DASHBOARD_MY_CARD_DAILY} renderedFrom = 'dashboard' /> :

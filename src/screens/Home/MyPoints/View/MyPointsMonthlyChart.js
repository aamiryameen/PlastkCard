...
import { DASHBOARD_MY_POINTS_MONTHLY, mulish_regular } from '../../../../utils/Constants';
...
export default MyPointsMonthlyChart = (props) => {
    ...
    return (
        ...
        <LinearGradient colors={[myTheme.colors.CARD_GRADIENT_FIRST_COLOR, myTheme.colors.CARD_GRADIENT_SECOND_COLOR]} style={styles.linearGradientContainerStyle}>
            ...
            <View style={{ flexDirection: 'column', marginBottom: normalizeY(30), marginTop: normalizeY(5), marginHorizontal: normalizeX(15) }}>
                <Text style={[styles.titleTextStyle, { color: myTheme.colors.LABEL_COLOR, textAlign: 'center' }]}>Points earned in the last 12 months</Text>
            ...
            {response !== '' ? response.label && response.label.length > 0 ? <LineChartView response={response} view={DASHBOARD_MY_POINTS_MONTHLY} renderedFrom = 'dashboard' /> :
